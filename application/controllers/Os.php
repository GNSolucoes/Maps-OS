...
class Os extends MY_Controller
{
    ...
    public function gerenciar()
    {
        ...
        if(count($where_array) > 0) {
            $this->data['configuration']['suffix'] = "?pesquisa={$pesquisa}&status={$status}&tipo={$tipo}&data={$inputDe}&data2={$inputAte}";
            $this->data['configuration']['first_url'] = base_url("index.php/os/gerenciar")."\?pesquisa={$pesquisa}&status={$status}&tipo={$tipo}&data={$inputDe}&data2={$inputAte}";
        }
    ...
    public function adicionar()
    {
        ...
        $this->data['templates'] = $this->checklist_model->getTemplates();
    ...
    public function autoCompleteEquipamento()
    {
        if (isset($_GET['term'])) {
            ...
            $this->os_model->autoCompleteEquipamento($q, $cliente_id);
        }
    }
    ...
    {
        $this->load->library('form_validation');
        $this->form_validation->set_rules('idEquipamento', 'Equipamento', 'required|trim|numeric');
        $this->form_validation->set_rules('idOsEquipamento', 'OS', 'required|trim|numeric');
        ...
        $data = [
            'equipamentos_id' => $this->input->post('idEquipamento'),
            'os_id' => $this->input->post('idOsEquipamento'),
        ];

        if ($this->os_model->add('equipamentos_os', $data) == true) {
            log_info('Adicionou equipamento a uma OS. ID (OS): ' . $this->input->post('idOsEquipamento'));
            return $this->output
                ->set_content_type('application/json')
                ->set_status_header(200)
                ->set_output(json_encode(['result' => true]));
    ...
    public function excluirEquipamento()
    {
        ...
        if ($this->os_model->delete('equipamentos_os', 'idEquipamentos_os', $id) == true) {
            log_info('Removeu equipamento de uma OS. ID (OS): ' . $idOs);
            echo json_encode(['result' => true]);
        } else {
            echo json_encode(['result' => false]);
        }
    ...
    public function rotas() {
        unset($this->data['menuOs']);
        $this->data['menuRotas'] = 'rotas';
        $this->data['view'] = 'tecnico/rotas';
        
        // Debug: Testando query
        ...
        try {
            ...
            $this->db->order_by('os.usuarios_id', 'asc');
            $this->db->order_by('os.idOs', 'desc');
            $query = $this->db->get();
            $this->data['rotas'] = $query ? $query->result() : [];
        } catch (Exception $e) {
            $this->data['rotas'] = [];
            log_message('error', 'Erro na query rotas: ' . $e->getMessage());
        }
    ...
    public function nova_os_rapida() {
        unset($this->data['menuOs']);
        $this->data['menuOsRapida'] = 'os_rapida';
        
        // Se for admin/gerente, mostra lista de OS Rápidas
        if ($this->permission->checkPermission($this->session->userdata('permissao'), 'cUsuario')) {
            $this->data['view'] = 'tecnico/lista_os_rapida';
            $this->db->select('os.*, clientes.nomeCliente, usuarios.nome as nomeTecnico');
            $this->db->from('os');
            $this->db->join('clientes', 'clientes.idClientes = os.clientes_id', 'left');
            $this->db->join('usuarios', 'usuarios.idUsuarios = os.usuarios_id', 'left');
            $this->db->where('os.os_rapida', 1);
            $this->db->order_by('os.idOs', 'desc');
            $this->data['results'] = $this->db->get()->result();
        } else {
            // Se for técnico, mostra o formulário de criação
    ...
    public function adicionar_os_rapida() {
        if (!$this->permission->checkPermission($this->session->userdata('permissao'), 'aOs')) {
        ...
        if ($this->form_validation->run() == false) {
            $this->session->set_flashdata('error', 'Selecione um cliente válido.');
            redirect(site_url('tecnico/nova_os_rapida'));
        }
        ...
        $dataOs = [
            'dataInicial' => date('Y-m-d'),
            'status' => $this->input->post('status') ?: 'Orçamento',
            'clientes_id' => $this->input->post('clientes_id'),
            'usuarios_id' => $this->input->post('usuarios_id') ?: $this->session->userdata('id_admin'),
            'descricaoProduto' => 'OS Rápida - Diversos',
            'defeito' => $this->input->post('defeito'),
            'laudoTecnico' => $this->input->post('laudo'),
            'tipo' => 'Interno',
            'os_rapida' => 1,
            'faturado' => 0
        ];
        ...
        $servicos_preco = $this->input->post('servicos_preco');
        $totalServicos = 0;

        if($servicos_nome){
            foreach($servicos_nome as $index => $nome){
                $preco = str_replace(',', '.', str_replace('R$ ', '', $servicos_preco[$index]));
                $idServico = $servicos_id[$index];

                // Se não tem ID, cria o serviço (Avulso)
                if(empty($idServico)){
                    $this->db->insert('servicos', [
                        'nome' => $nome,
                        'descricao' => 'Serviço Avulso criado via OS Rápida',
                        'preco' => $preco
                    ]);
        ...
        $produtos_qtd = $this->input->post('produtos_qtd');
        $totalProdutos = 0;

        if($produtos_id){
            foreach($produtos_id as $index => $idProd){
                $qtd = $produtos_qtd[$index];
                
                // Busca preço atual
                $this->db->where('idProdutos', $idProd);
                ...
                if($prod){
                    ...
                    // Baixa estoque
                    if($this->data['configuration']['control_estoque']){
                        $this->produtos_model->updateEstoque($idProd, $qtd, '-');
                    }
        ...
        $valorTotalOS = $totalServicos + $totalProdutos;

        // Se status finalizar, gera lançamento
        if($this->input->post('status') == 'Finalizado'){
             $dataLancamento = [
                'descricao' => "OS Rápida #" . $idOs,
                'valor' => $valorTotalOS,
                'tipo' => 'receita',
                ...
                'forma_pgto' => $this->input->post('formaPgto') ?: 'Dinheiro',
                'usuarios_id' => $this->session->userdata('id_admin'),
                'clientes_id' => $this->input->post('clientes_id'),
            ];
            ...
            $this->db->where('idOs', $idOs);
            $this->db->update('os', ['faturado' => 1, 'valorTotal' => $valorTotalOS]);
        }

        $this->db->trans_complete();

        if ($this->db->trans_status() === FALSE) {
            $this->session->set_flashdata('error', 'Erro ao processar OS rápida.');
        } else {
            $this->session->set_flashdata('success', 'OS rápida salva com sucesso!');
        }

        redirect(site_url('tecnico/nova_os_rapida'));
    }

    public function migration_add_tipo() {
        $this->load->dbforge();
        $added = 0;
        
        if (!$this->db->field_exists('tipo', 'os')) {
            $fields = [
                'tipo' => [
                    'type' => 'VARCHAR',
                    'constraint' => '20',
            ...
            echo "Field 'tipo' added successfully.<br>";
            $added++;
        }

        if (!$this->db->field_exists('os_rapida', 'os')) {
            $fields = [
                'os_rapida' => [
                    'type' => 'TINYINT',
                    'constraint' => '1',
            ...
            $this->dbforge->add_column('os', $fields);
            echo "Field 'os_rapida' added successfully.<br>";
            $added++;
        }
    ...
    public function getChecklistItems()
    {
        ...
        $items = $this->checklist_model->getItemsByTemplate($template_id);
