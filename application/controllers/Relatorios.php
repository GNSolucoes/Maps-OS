...
class Relatorios extends MY_Controller
{
    ...
    {
        if (! $this->permission->checkPermission($this->session->userdata('permissao'), 'rCliente')) { // Using rCliente as proxy permission or create new one
            $this->session->set_flashdata('error', 'Você não tem permissão para gerar relatórios de checklists.');
            redirect(base_url());
        }
        ...
        $this->data['view'] = 'relatorios/rel_checklists';

        return $this->layout();
    }

    public function checklistsRapid()
    {
        if (! $this->permission->checkPermission($this->session->userdata('permissao'), 'rCliente')) {
            $this->session->set_flashdata('error', 'Você não tem permissão para gerar relatórios de checklists.');
            redirect(base_url());
        ...
        $this->load->model('checklist_model');
        $data['checklists'] = $this->Relatorios_model->checklistsRapid();
        $data['emitente'] = $this->Mapos_model->getEmitente();
        $data['title'] = 'Relatório de Checklists';
        $data['topo'] = $this->load->view('relatorios/imprimir/imprimirTopo', $data, true);

        $this->load->helper('mpdf');
        $html = $this->load->view('relatorios/imprimir/imprimirChecklists', $data, true);
        pdf_create($html, 'relatorio_checklists' . date('d/m/y'), true);
    }

    public function checklistsCustom()
    {
        if (! $this->permission->checkPermission($this->session->userdata('permissao'), 'rCliente')) {
            $this->session->set_flashdata('error', 'Você não tem permissão para gerar relatórios de checklists.');
            redirect(base_url());
        ...
        $data['checklists'] = $this->Relatorios_model->checklistsCustom($dataInicial, $dataFinal, $responsavel, $template);
        $data['emitente'] = $this->Mapos_model->getEmitente();
        $data['title'] = 'Relatório de Checklists Customizado';
        $data['topo'] = $this->load->view('relatorios/imprimir/imprimirTopo', $data, true);
    ...
    {
        if (! $this->permission->checkPermission($this->session->userdata('permissao'), 'rCliente')) {
            $this->session->set_flashdata('error', 'Você não tem permissão para gerar relatórios de parceiros.');
            redirect(base_url());
        }
        $this->load->model('Parceiros_model');
        $this->data['parceiros'] = $this->Parceiros_model->get('parceiros', '*', 'situacao = 1');
        $this->data['view'] = 'relatorios/rel_parceiros';
        return $this->layout();
    }

    public function parceirosRapid()
    {
        if (! $this->permission->checkPermission($this->session->userdata('permissao'), 'rCliente')) {
            $this->session->set_flashdata('error', 'Você não tem permissão para gerar relatórios de parceiros.');
            redirect(base_url());
        ...
        $dataInicial = $this->input->get('dataInicial');
        $dataFinal = $this->input->get('dataFinal');
        $parceiroId = $this->input->get('parceiro_id');
        $gerarLancamento = $this->input->get('gerar_lancamento');

        $this->load->model('Relatorios_parceiros_model');
        $this->data['results'] = $this->Relatorios_parceiros_model->getComissoes($dataInicial, $dataFinal, $parceiroId);
        $this->data['dataInicial'] = $dataInicial;
        $this->data['dataFinal'] = $dataFinal;
        
        // Logica de Geração de Lançamento Financeiro
        if ($gerarLancamento == 1 && !empty($this->data['results'])) {
            $this->load->model('financeiro_model');
            
            // Agrupar por parceiro
            $comissoesPorParceiro = [];
            foreach ($this->data['results'] as $r) {
                ...
                $chave = $r->nomeParceiro; // Usando nome como chave
                if (!isset($comissoesPorParceiro[$chave])) {
                    $comissoesPorParceiro[$chave] = 0;
                }
                $comissoesPorParceiro[$chave] += $vlrComissao;
            }
            
            foreach ($comissoesPorParceiro as $parceiroNome => $valorTotal) {
                if ($valorTotal > 0) {
                    $dados = [
                        'descricao' => 'Comissão - ' . $parceiroNome . ' (' . date('d/m', strtotime($dataInicial)) . ' a ' . date('d/m', strtotime($dataFinal)) . ')',
                        'valor' => $valorTotal,
                        'data_vencimento' => date('Y-m-d'),
                        'data_pagamento' => null,
                        'baixado' => 0,
                        'cliente_fornecedor' => $parceiroNome,
                        'forma_pgto' => 'Dinheiro', // Padrão
                        'tipo' => 'despesa',
                        'observacoes' => 'Gerado automaticamente pelo relatório de comissões.',
                        'usuarios_id' => $this->session->userdata('id') // Se a tabela pedir
                    ];
                    // O model 'add' do financeiro espera (tabela, dados).
                    // Vou assumir insert direto se o model nao tiver metodo especifico validado agora.
                    // Financeiro_model geralmente usa metodos padrao MY_Model 'add'.
                    $this->financeiro_model->add('lancamentos', $dados);
                }
            }
        }

        $this->load->helper('mpdf');
        $html = $this->load->view('relatorios/imprimir/imprimirParceiros', $this->data, true);
        pdf_create($html, 'Relatorio_Comissoes_Parceiros_' . date('d_m_y'), true);
    }
}
