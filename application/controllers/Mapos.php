...
class Mapos extends MY_Controller {
    ...
    public function index()
    {
        ...
        // Logic for Minhas Rotas
        // Logic for Minhas Rotas
        $this->db->select('os.idOs, os.status, clientes.nomeCliente, clientes.rua, clientes.numero, clientes.bairro, clientes.cidade');
        $this->db->from('os');
        ...
        $query = $this->db->get();
        $this->data['minhas_rotas'] = $query ? $query->result() : [];

        // Logic for Clientes Map
        $this->db->select('idClientes, nomeCliente, rua, numero, bairro, cidade, cep');
        $this->db->limit(50); // Limit to avoid performance issues
        $this->db->order_by('idClientes', 'DESC');
        $this->data['clientes_mapa'] = $this->db->get('clientes')->result();
    ...
    public function enviarEmailAgora()
    {
        if (!$this->permission->checkPermission($this->session->userdata('permissao'), 'cEmail')) {
        ...
        $this->load->model('email_model');
        $fila = $this->email_model->get('email_queue', '*', "id = $id", 1, 0);
        $email = !empty($fila) ? $fila[0] : null;
        ...
        if($email->status == 'sent') {
             $this->session->set_flashdata('error', 'Este email já foi enviado.');
             redirect(base_url() . 'index.php/mapos/emails');
        }
        ...
        if($this->email->send()){
             // $this->email_model->delete('email_queue', 'id', $id);
             $this->email_model->edit('email_queue', ['status' => 'sent', 'date' => date('Y-m-d H:i:s')], 'id', $id);
             $this->session->set_flashdata('success', 'Email enviado com sucesso!');
        } else {
             $this->email_model->edit('email_queue', ['status' => 'failed', 'date' => date('Y-m-d H:i:s')], 'id', $id);
             $this->session->set_flashdata('error', 'Erro ao enviar email: ' . $this->email->print_debugger());
        }
        redirect(base_url() . 'index.php/mapos/emails');
    }
    ...
    public function configurar()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $data = [
                ...
                'tawk_to_embed' => $this->input->post('tawk_to_embed'),
            ];
