...
class Github_updater
{
    ...
    {
        try {
            $repoUrl = self::GITHUB_URL . $this->ci->config->item('github_user') . '/' . $this->ci->config->item('github_repo') . '.git';
            $rootPath = FCPATH;
            chdir($rootPath);
            
            // Tenta configurar remote se necessário
            if (!is_dir('.git')) {
                shell_exec('git init');
                shell_exec('git remote add origin ' . $repoUrl);
            } else {
                shell_exec('git remote set-url origin ' . $repoUrl);
            }
            
            shell_exec('git fetch origin');
            
            // Tenta reset hard para garantir que estamos iguais ao remote (sobrescreve local)
            // Tenta main primeiro, se falhar tenta master
            ...
            log_message('info', 'Git Update Output: ' . $output);
            
            // Atualiza versão no config se possível (opcional, já que o arquivo config.php virá do git também)
            // Mas o arquivo config.php muitas vezes é ignorado ou específico do ambiente.
            // Se o usuário clonar o repo, ele terá o config.php do repo.
            
            return true;
        } catch (Exception $e) {
            log_message('error', 'Git Update Error: ' . $e->getMessage());
            return false;
        }
    }
