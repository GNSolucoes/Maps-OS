...
class Relatorios_model extends CI_Model
{
    ...
    {
        $this->db->select('checklist_os.*, checklist_templates.nome as template_nome, usuarios.nome as usuario_nome, os.idOs');
        $this->db->from('checklist_os');
        $this->db->join('checklist_templates', 'checklist_templates.id = checklist_os.template_id');
        $this->db->join('usuarios', 'usuarios.idUsuarios = checklist_os.usuario_id');
        $this->db->join('os', 'os.idOs = checklist_os.os_id', 'left');
        $this->db->order_by('checklist_os.data_cadastro', 'desc');
        $this->db->limit(100);

        return $this->db->get()->result();
    }

    public function checklistsCustom($dataInicial = null, $dataFinal = null, $responsavel = null, $template = null)
    {
        $this->db->select('checklist_os.*, checklist_templates.nome as template_nome, usuarios.nome as usuario_nome, os.idOs');
        $this->db->from('checklist_os');
        $this->db->join('checklist_templates', 'checklist_templates.id = checklist_os.template_id');
        ...
        if ($dataInicial) {
            $this->db->where('checklist_os.data_cadastro >=', $dataInicial);
        }
        if ($dataFinal) {
            $this->db->where('checklist_os.data_cadastro <=', $dataFinal . ' 23:59:59');
        }
        if ($responsavel) {
            $this->db->where('checklist_os.usuario_id', $responsavel);
        }
        if ($template) {
            $this->db->where('checklist_os.template_id', $template);
        }
