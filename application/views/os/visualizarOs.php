            ...
            <div class="widget-title" style="margin: 10px 0 0">
                <div class="buttons">
                    ...
                    <?php if ($this->permission->checkPermission($this->session->userdata('permissao'), 'vOs')) {
                        ...
                        if (!empty($zapnumber)) {
                            echo '<a title="Enviar Por WhatsApp Web" class="button btn btn-mini btn-success" id="enviarWhatsApp" target="_blank" href="https://api.whatsapp.com/send?phone=55' . $zapnumber . '&text=' . $texto_de_notificacao . '">
                            ...
                            if(!empty($_ENV['WHATICKET_API_URL']) && !empty($_ENV['WHATICKET_API_TOKEN'])) {
                                echo '<a title="Enviar via Whaticket API" class="button btn btn-mini btn-info" id="enviarWhatsAppApi" href="'.site_url().'/os/enviar_whatsapp_api/'.$result->idOs.'">
                                            ...
                                            <?php if (!empty($result->celular_cliente) || !empty($result->telefone_cliente) || !empty($result->contato_cliente)): ?>
                                                <span><i class='bx bxs-phone'></i>
                                                    ...
                                                    <?php if ($result->celular_cliente == $result->telefone_cliente) { ?>
                                                        ...
                                                        <a href="<?= whatsapp_link($result->celular_cliente) ?>" target="_blank" title="WhatsApp"><i class="bx bxl-whatsapp" style="color:green"></i></a>
                                                    <?php } else { ?>
                                                        <?= !empty($result->telefone_cliente) ? $result->telefone_cliente : "" ?>
                                                        <?= !empty($result->telefone_cliente) ? '<a href="'.whatsapp_link($result->telefone_cliente).'" target="_blank" title="WhatsApp"><i class="bx bxl-whatsapp" style="color:green"></i></a>' : "" ?>
                                                        <?= !empty($result->celular_cliente) && !empty($result->telefone_cliente) ? ' / ' : "" ?>
                                                        <?= !empty($result->celular_cliente) ? $result->celular_cliente : "" ?>
                                                        <?= !empty($result->celular_cliente) ? '<a href="'.whatsapp_link($result->celular_cliente).'" target="_blank" title="WhatsApp"><i class="bx bxl-whatsapp" style="color:green"></i></a>' : "" ?>
                                                    <?php } ?>
                                                </span></br>
                                            <?php endif; ?>
                                            <?php
