...
<style type="text/css">
            ...
            <div class="widget-content">
                <div class="row-fluid">
                    <?php 
                        ...
                        foreach($contas as $c): 
                        $saldo = $this->contas_model->getSaldoAtual($c->idConta);
                    ?>
                    <div class="span3 stat-box" style="margin-bottom: 10px; background: #f9f9f9; padding: 10px; border: 1px solid #ddd; border-radius: 4px;">
                        <div class="stat-icon" style="float:left; margin-right: 10px; font-size: 24px; color: #2c3e50;">
                            <i class="fas fa-wallet"></i>
                        </div>
                        <div class="stat-content">
                            <strong style="display:block; font-size: 14px;"><?php echo $c->conta; ?></strong>
                            <span style="font-size: 16px; font-weight: bold; color: <?php echo $saldo >= 0 ? 'green' : 'red'; ?>">R$ <?php echo number_format($saldo, 2, ',', '.'); ?></span>
                        </div>
                    </div>
                    <?php endforeach; ?>
                </div>
    ...
    <div class="widget-title">
                ...
                <h5>
                <?php 
                    $filtroTipo = $this->input->get('tipo');
    ...
    <?php if ($this->permission->checkPermission($this->session->userdata('permissao'), 'aLancamento')) { ?>
        <div class="" style="display:flex">
            <a href="#modalReceita" data-toggle="modal" role="button" class="button btn btn-mini btn-success" style="width: 230px" onclick="setTipoModal('<?= $this->input->get('tipo') ?>')">
                <span class="button__icon"><i class='bx bx-plus-circle'></i></span><span class="button__text2" title="Cadastrar nova receita ou despesa"> 
                <?php 
                    if($this->input->get('tipo') == 'receita'){ echo 'Adicionar Receita'; }
                    elseif($this->input->get('tipo') == 'despesa'){ echo 'Adicionar Despesa'; }
                    else { echo 'Receita/Despesa'; }
                ?>
                </span></a>
        </div>
    <?php } ?>
    ...
    <div class="span12" style="margin-left: 0;margin-top: 1rem;">
        <form action="<?php echo current_url(); ?>" method="get">
            ...
            <div class="span2" style="<?= ($this->input->get('tipo') ? 'display:none' : '') ?>">
                <label>Tipo</label>
            ...
            <?php if($this->input->get('tipo')) { ?>
                <input type="hidden" name="tipo" value="<?= $this->input->get('tipo') ?>">
            <?php } ?>
            ...
            <div class="widget-content nopadding tab-content">
                ...
                <table class="table table-bordered " id="divLancamentos">
                    <thead>
                        <tr>
                            <th>#</th>
                            <?php if(!$this->input->get('tipo')){ ?><th>Tipo</th><?php } ?>
                            <th>Cliente / Fornecedor</th>
                            <th>Descrição</th>
                            <th>Vencimento</th>
                            <th>Data Pgto.</th>
                            <th>Status</th>
                            <?php if(!$this->input->get('tipo')){ ?><th>Observações</th><?php } ?>
                            <?php if(!$this->input->get('tipo')){ ?><th>Forma de Pagamento</th><?php } ?>
                            <th>Valor (+)</th>
                            <?php if(!$this->input->get('tipo')){ ?><th>Desconto (-)</th><?php } ?>
                            <th>Valor Total (=)</th>
                            <th>Ações</th>
                        </tr>
                    </thead>
              ...
              <td colspan="9" >Nenhum lançamento encontrado</td>
                        ...
                        $soma_receita_paga = 0;
                        $soma_receita_pendente = 0;
                        $soma_despesa_paga = 0;
                        $soma_despesa_pendente = 0;

                        foreach ($results as $r) {
                            $vencimento = date(('d/m/Y'), strtotime($r->data_vencimento));
                            
                            $valor_final = $r->valor_desconto != 0 ? $r->valor_desconto : $r->valor;
                            if ($r->tipo == 'receita') {
                               if ($r->baixado == 1) $soma_receita_paga += $valor_final;
                               else $soma_receita_pendente += $valor_final;
                            } else {
                               if ($r->baixado == 1) $soma_despesa_paga += $valor_final;
                               else $soma_despesa_pendente += $valor_final;
                            }
                            $data_pgto_show = ($r->baixado && $r->data_pagamento != '0000-00-00') ? date(('d/m/Y'), strtotime($r->data_pagamento)) : '-';
                            ...
                            if(!$this->input->get('tipo')){ echo '<td><span class="label label-' . $label . '">' . ucfirst($r->tipo) . '</span></td>'; }
                            echo '<td>' . $r->cliente_fornecedor . '</td>';
                            echo '<td>' . $r->descricao . '</td>';
                            echo '<td>' . $vencimento . '</td>';
                            echo '<td>' . $data_pgto_show . '</td>';
                            echo '<td>' . $status . '</td>';
                            if(!$this->input->get('tipo')){ echo '<td>' . $r->observacoes . '</td>'; }
                            if(!$this->input->get('tipo')){ echo '<td>' . $r->forma_pgto . '</td>'; }
                            echo '<td> R$ ' . number_format($r->valor, 2, ',', '.') . '</td>'; //valor total sem o desconto
                            if(!$this->input->get('tipo')){ echo  $r->tipo_desconto == "real" ? '<td>' . "R$ ".$r->desconto . '</td>' : ($r->tipo_desconto == "porcento" ? '<td>' . $r->desconto." %" . '</td>' : '<td>' . "0" . '</td>'); } // valor do desconto
                    ...
                    <tfoot>
                        <?php if($this->input->get('tipo') == 'receita' || !$this->input->get('tipo')) { ?>
                        <tr>
                            <td colspan="5" style="text-align: right; color: green"><strong>Total Recebido:</strong></td>
                            <td colspan="5" style="text-align: left; color: green">
                                <strong>R$ <?php echo number_format($soma_receita_paga, 2, ',', '.') ?></strong>
                            </td>
                        </tr>
                         <tr>
                            <td colspan="5" style="text-align: right; color: #E91E63"><strong>Total a Receber:</strong></td>
                            <td colspan="5" style="text-align: left; color: #E91E63">
                                <strong>R$ <?php echo number_format($soma_receita_pendente, 2, ',', '.') ?></strong>
                            </td>
                        </tr>
                        <tr>
                            <td colspan="5" style="text-align: right; color: black"><strong>Total Geral Receitas:</strong></td>
                            <td colspan="5" style="text-align: left; color: black">
                                <strong>R$ <?php echo number_format($soma_receita_paga + $soma_receita_pendente, 2, ',', '.') ?></strong>
                            </td>
                        </tr>
                        <?php } ?>
                        
                        <?php if($this->input->get('tipo') == 'despesa' || !$this->input->get('tipo')) { ?>
                        <tr>
                            <td colspan="5" style="text-align: right; color: red"><strong>Total Pago:</strong></td>
                            <td colspan="5" style="text-align: left; color: red">
                                <strong>R$ <?php echo number_format($soma_despesa_paga, 2, ',', '.') ?></strong>
                            </td>
                        </tr>
                         <tr>
                            <td colspan="5" style="text-align: right; color: #E91E63"><strong>Total a Pagar:</strong></td>
                            <td colspan="5" style="text-align: left; color: #E91E63">
                                <strong>R$ <?php echo number_format($soma_despesa_pendente, 2, ',', '.') ?></strong>
                            </td>
                        </tr>
                        <tr>
                            <td colspan="5" style="text-align: right; color: black"><strong>Total Geral Despesas:</strong></td>
                            <td colspan="5" style="text-align: left; color: black">
                                <strong>R$ <?php echo number_format($soma_despesa_paga + $soma_despesa_pendente, 2, ',', '.') ?></strong>
                            </td>
                        </tr>
                        <?php } ?>
                        ...
                        <?php if(!$this->input->get('tipo')) { ?>
                        <tr>
                            <td colspan="5" style="text-align: right"><strong>Saldo (Recebido - Pago):</strong></td>
                            <td colspan="5" style="text-align: left;">
                                <strong>R$ <?php echo number_format($soma_receita_paga - $soma_despesa_paga, 2, ',', '.') ?></strong>
                            </td>
                        </tr>
                        <?php } ?>
                        <?php if(!$this->input->get('tipo')) { ?>
    ...
    <form id="formReceita" action="<?php echo base_url() ?>index.php/financeiro/adicionarReceita" method="post">
        ...
        <div class="modal-body">
            ...
            <div class="span3" style="margin-left: 2">
                ...
                <select name="contas_bancaria_id" id="conta" class="span12">
                    <?php foreach($contas as $c): ?>
                        <option value="<?php echo $c->idConta; ?>"><?php echo $c->conta; ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
...
<div id="modalReceitaParcelada" class="modal hide fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
  ...
  <div class="modal-body">	
          ...
          <div class="span3" style="margin-left: 0">
            ...
            <div class="span3" style="margin-left: 2">
                <label for="conta_parc">Conta Bancária</label>
                <select name="contas_bancaria_id_parc" id="conta_parc" class="span12">
                    <?php foreach($contas as $c): ?>
                        <option value="<?php echo $c->idConta; ?>"><?php echo $c->conta; ?></option>
                    <?php endforeach; ?>
                </select>
    ...
    <form id="formEditar" action="<?php echo base_url() ?>index.php/financeiro/editar" method="post">
        ...
        <div class="span4">  
            ...
            <input class="span12 money" id="descontoEditar" name="valor_desconto_editar" type="text" value="<?php echo number_format("0.00", 2, ',', '.') ?>" />
                ...
                <div class="span4">
                    <label for="contaEditar">Conta Bancária</label>
                    <select name="contas_bancaria_id" id="contaEditar" class="span12">
                        <?php foreach($contas as $c): ?>
                            <option value="<?php echo $c->idConta; ?>"><?php echo $c->conta; ?></option>
                        <?php endforeach; ?>
                    </select>
