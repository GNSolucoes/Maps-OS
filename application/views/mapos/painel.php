    ...
    <div class="span6">
        <div class="widget-box">
             <div class="widget-title">
                ...
                <h5>Minhas Rotas (OS Abertas)</h5>
             </div>
             <div class="widget-content" style="height: 300px; overflow-y: scroll">
                 <table class="table table-bordered">
                     ...
                     <tbody>
                         <?php if(!isset($minhas_rotas) || !$minhas_rotas){ ?>
                             <tr><td colspan="4">Nenhuma rota pendente.</td></tr>
                         <?php } else { foreach($minhas_rotas as $r){ ?>
                             <tr class="rota-item" data-endereco="<?php echo $r->rua . ', ' . $r->numero . ', ' . $r->cidade; ?>" data-cliente="<?php echo $r->nomeCliente; ?>">
                                 <td>#<?php echo $r->idOs; ?></td>
                                 <td><?php echo $r->nomeCliente; ?></td>
                                 <td><?php echo $r->rua . ', ' . $r->numero; ?></td>
                                 <td>
                                     <a href="https://www.google.com/maps/dir/?api=1&destination=<?php echo urlencode($r->rua . ', ' . $r->numero . ', ' . $r->cidade . ' - ' . $r->bairro); ?>" target="_blank" class="btn btn-mini btn-primary" title="Navegar"><i class='bx bxl-google'></i></a>
                                 </td>
                             </tr>
                         <?php }} ?>
                     </tbody>
...
document.addEventListener('DOMContentLoaded', function() {
    var map = L.map('dashboard-map').setView([-14.235004, -51.92528], 4); // Center of Brazil

    L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', {
        attribution: '&copy; <a href="https://www.openstreetmap.org/copyright">OpenStreetMap</a> contributors'
    }).addTo(map);

    // Try to geocode and plot routes
    var rotas = document.querySelectorAll('.rota-item');
    var delay = 0;
    
    if(rotas.length > 0) {
        var group = new L.featureGroup();
        
        rotas.forEach(function(row) {
            var address = row.getAttribute('data-endereco');
            var client = row.getAttribute('data-cliente');
                ...
                .then(data => {
                    if (data && data.length > 0) {
                        ...
                        var marker = L.marker([lat, lon]).addTo(map)
                            .bindPopup('<b>' + client + '</b><br>' + address);
            ...
            delay += 1200; // 1.2s delay to respect API limits
        });
    }
});
</script>
